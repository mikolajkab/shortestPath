...
{ 
	...
	deque<liPair> node_queue;
	node_queue.push_front(make_pair(graph->nodes[src], src));
	...
	while(!node_queue.empty())
	{
		...
		for (i = node.begin(); i != node.end(); ++i)
		{
			...
			if ((dist[u] != INT_MAX) && (dist[u] + weight < dist[v])) 
			{
				...
				{
					if(dist[v] <= dist[node_queue.front().second])
                    {
                        node_queue.push_front(make_pair(graph->nodes[v], v));
                    }
                    else
                    {
                        node_queue.push_back(make_pair(graph->nodes[v], v));
                    }                    
					node_set.insert(v);
				}
			}
