...
def generate_qraph():
    ...
    print("number of possible states: ", len(x1_list)*len(x2_list)*len(x3_list))
    ...
    for u1 in u1_list:
        for u2 in u2_list:
            for u3 in u3_list:
                ...
                x3_k1 = c31*x3_k + c32*sqrt(x2_k) + c33*sqrt(x3_k) + c34*u3

                # round to closest state
                x1_k_1 = round(take_closest(x1_list, x1_k1), 5)
                x2_k_1 = round(take_closest(x2_list, x2_k1), 5)
                x3_k_1 = round(take_closest(x3_list, x3_k1), 5)
                
                # check if x_k_1 is within allowed ranges
                if      (x1_k_1 >= x1_min and x1_k_1 <= x1_max) \
                    and (x2_k_1 >= x2_min and x2_k_1 <= x2_max) \
                    and (x3_k_1 >= x3_min and x3_k_1 <= x3_max):

                    x_k_1_tup = (x1_k_1, x2_k_1, x3_k_1)

                    if x_k_1_tup not in states:

                        x_k_1 = np.matrix([[x1_k_1-end_state[0]], [x2_k_1-end_state[1]], [x3_k_1-end_state[2]]])
                        u = np.matrix([[u1-u_end[0]], [u2-u_end[1]], [u3-u_end[2]]])
...
lqr_initial_state = (initial_state[0]-end_state[0], initial_state[1]-end_state[1], initial_state[2]-end_state[2])
