...
import graph as gr
from plant_tanks2_one_input import *
import pickle
...
def generate_qraph():
    ...
    # print("states: ", sorted(visited.item(0)))
...
def generate_adjacent_nodes(actual):
    ...
    for u1 in u1_list:
        ...
        x2_k1 = c21*x2_k + c22*math.sqrt(x1_k) + c23*math.sqrt(x2_k)
        ...
        if      (x1_k_1 >= x1_min and x1_k_1 <= x1_max) \
            ...
            if x_k_1_tup not in states:
                ...
                cost = x_k_1.T * Q * x_k_1 + u1.T * R * u1
...
x1_lqr, x2_lqr, u1_lqr = solve_lqr(lqr_initial_state)
